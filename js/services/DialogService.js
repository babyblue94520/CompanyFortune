...
//		'	<div class="common-dialog-screen"></div>{0}',
		'	{0}',
		'</div>'
	].join('');
	...
	function _hideHandler(){
		...
		that.$el.find('.common-dialog-model').css({top:_defaultTop,opacity:0});
	...
	function _resizeHandler(){
		...
		var elHeight = that.$el.find('.common-dialog-model').height();
		var top = windowHeight<elHeight?20:(windowHeight-elHeight)/2;
...
//		}
		setTimeout(function(){
			that.$el.find('.common-dialog-model').css({top:top,opacity:1});
		},50);
	}
...
function DialogService(){
	...
	var _dialogConfigs = {
		...
		'goGameOrFunGame':{id:'goGameOrFunGame',initFn:false,defaultSetFn:false,html:false}
	...
	_dialogConfigs['login'].html = [
		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
     		...
     		'<div class="mui-panel">',
     			'<div class="common-dialog-error-message"></div>',
    			...
    			'<div class="mui-buttons">',
     		  	...
     		  	'	<div class="mui-btn mui-btn--raised mui-btn--dark small" id="dialog-login-forgotPassword-btn">忘记密码</div>',
     		  	'</div>',
     		'</div>',
     	'</div>'
    ].join('');
	...
	_dialogConfigs['login'].initFn = function(dialog){
		...
		dialog.$errorMessage = dialog.$el.find('.common-dialog-error-message');
		...
		dialog.$image.click(function(){
			//刷新验证图
			webManage.changeValidateImage(dialog.$image);
		});
		
		dialog.$forgotPasswordBtn.click(function(){
			dialog.hide();
			webManage.loadView('forgotPassword');
		});
		...
		dialog.$submitBtn.click(function(){
			...
			webManage.getLoader().open('验证中',_zIndex);
			webManage.getUserService().login(formData, function(result){
				if(result.success){
					alert(result.message);
					dialog.hide();
					console.log(dialog.param)
					if(dialog.param&&dialog.param.redirect){
						webManage.loadView(dialog.param&&dialog.param.redirect);
					}
				}else{
					dialog.$code.val('');
					//刷新验证图
		...
		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
    		...
    		'<div class="mui-panel">',
    			...
    			'<div class="mui-buttons">',
    		  	...
    		  	'	<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-logout-cancel-btn">取消</div>',
    		  	'</div>',
    		'</div>',
    	'</div>'
   ].join('');
	...
	_dialogConfigs['logout'].initFn = function(dialog){
		...
		dialog.$el.find('#dialog-logout-submit-btn').click(function(){
			webManage.getLoader().open('登出中',_zIndex);
			webManage.getUserService().logout( function(result){
				webManage.getLoader().close();
				if(result.success){
					dialog.hide();
				}else{
		...
		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
     		...
     		'<div class="mui-panel">',
    			...
    			'<div class="mui-buttons">',
     		  	...
     		  	'	<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-makeCall-cancel-btn">取消</div>',
     		  	'</div>',
     		'</div>',
     	'</div>'
    ].join('');
	...
	_dialogConfigs['makeCall'].initFn = function(dialog){
		...
		dialog.$submitBtn.click(function(){
			...
			webManage.getLoader().open('处理中',_zIndex);
			webManage.getUserService().makeCall(formData, function(result){
				if(result.success){
					alert(result.message);
				}else{
					alert(result.message);
		...
		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
     		...
     		'<div class="mui-panel">',
    			...
    			'<div class="mui-buttons">',
     		  	...
     		  	'	<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-modifyPassword-cancel-btn">取消</div>',
     		  	'</div>',
     		'</div>',
     	'</div>'
    ].join('');
	...
	_dialogConfigs['modifyPassword'].initFn = function (dialog){
		...
		dialog.$submitBtn.click(function(){
			...
			webManage.getLoader().open("修改中",_zIndex);
			webManage.getUserService().changePassword(formData, function(result){
				webManage.getLoader().close();
				if(result.success){
					alert(result.message);
					dialog.hide();
		...
		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
     		...
     		'<div class="mui-panel">',
    			...
    			'<div class="mui-buttons">',
     		  	...
     		  	'	<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-question-cancel-btn">取消</div>',
     		  	'</div>',
     		'</div>',
     	'</div>'
    ].join('');
	...
	_dialogConfigs['question'].initFn = function(dialog){
		...
		dialog.$el.find('#dialog-question-submit-btn').click(function(){
			...
			webManage.getLoader().open("设置中",_zIndex);
			webManage.getUserService().saveQuestion(formData, function(result){
				webManage.getLoader().close();
				if(result.success){
					alert(result.message);
					dialog.hide();
     	...
     	'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
     		...
     		'<div class="mui-panel">',
    			...
    			'<div class="mui-buttons">',
     		  	...
     		  	'	<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-download-cancel-btn">取消</div>',
     		  	'</div>',
     		'</div>',
     	'</div>'
 	].join('');
	...
	_dialogConfigs['bankBind'].html = [
		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
     		...
     		'<div class="mui-panel">',
     			...
     			'<div class="common-dialog-error-message">注：只可以绑定三个银行卡/折号，且每个银行只可绑定一个卡号。如须解绑，请与在线客服联系。</div>',
     		  	'<div class="mui-textfield mui-textfield--float-label">',
	...
	_dialogConfigs['bankBind'].initFn = function(dialog){
		...
		dialog.$submitBtn.click(function(){
			...
			webManage.getLoader().open("绑定中",_zIndex);
			webManage.getBankService().bindBankNo(formData, function(result){
				webManage.getLoader().close();
				if(result.success){
					alert(result.message);
					dialog.hide();
     	...
     	'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
     		...
     		'<div class="mui-panel">',
    			...
    			'<div class="mui-buttons">',
     		  	...
     		  	'	<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-goGame-cancel-btn">取消</div>',
     		  	'</div>',
     		'</div>',
     	'</div>'
 	].join('');
	...
	_dialogConfigs['goGame'].initFn = function(dialog){
		...
		dialog.$error = dialog.$el.find('.common-dialog-error-message');
		...
		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1">',
    		...
    		'<div class="mui-panel">',
    			'<div class="common-dialog-message">',
		            ...
		            '详细活动内容，请至 <div style="color:#2C6AE0;text-decoration: underline;display: inline-block;" onclick="webManage.loadView(\'preferential\')">优惠活动</div>查询。',
    			'</div>',
    			'<div class="mui-buttons">',
    		  		'<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-sign-submit-btn">签到</div>',
    			'</div>',
    		'</div>',
    	'</div>'
   ].join('');
	...
	_dialogConfigs['sign'].initFn = function(dialog){
		...
		dialog.$el.find('#dialog-sign-submit-btn').click(function(){
			webManage.getLoader().open('处理中',_zIndex);
			webManage.getSignService().doSign( function(result){
				if(result.success){
					alert(result.message);
					dialog.hide();
				}else{
	...
	_dialogConfigs['zfbBind'].initFn = function(dialog){
		...
		dialog.$submitBtn.click(function(){
			webManage.getLoader().open("绑定中",_zIndex);
			webManage.getBankService().bindZFBQR({
		...
		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1 mui-col-xs48-8 mui-col-xs48-offset-2 mui-col-xs64-6 mui-col-xs64-offset-3 mui-col-sm-6 mui-col-sm-offset-3 mui-col-md-4 mui-col-md-offset-4">',
    		...
    		'<div class="mui-panel">',
    			...
    			'<div class="mui-buttons">',
    		  		...
    		  		'<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-confirm-cancel-btn">取消</div>',
    		  	'</div>',
    		'</div>',
    	'</div>'
    ].join('');
	...
	_dialogConfigs['selectSlot'].html = [
			'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1 mui-col-xs64-8 mui-col-xs64-offset-2 mui-col-sm-6 mui-col-sm-offset-3 mui-col-md-4 mui-col-md-offset-4">',
     		...
     		'<div class="mui-panel">',
    			...
    			'<div class="mui-buttons">',
     		  	'	<div class="mui-btn mui-btn--raised mui-btn--primary" id="dialog-selectSlot-submit-btn">领取</div>',
     		  	'	<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-selectSlot-cancel-btn">关闭</div>',
     		  	'</div>',
     		'</div>',
     	'</div>'
    ].join('');
	...
	_dialogConfigs['goGameOrFunGame'].html = [
  		'<div class="common-dialog-model mui-col-xs32-10 mui-col-xs32-offset-1 mui-col-xs48-8 mui-col-xs48-offset-2 mui-col-xs64-6 mui-col-xs64-offset-3 mui-col-sm-6 mui-col-sm-offset-3 mui-col-md-4 mui-col-md-offset-4">',
  		'<div class="common-dialog-title"><div id="dialog-goGameOrFunGame-title" class="common-dialog-title-text"></div><div class="common-dialog-close-btn flaticon-shapes"></div></div>',
    		'<div class="mui-panel">',
    			'<div id="dialog-goGameOrFunGame-message" class="mui-message" ></div>',
    			'<div class="mui-buttons">',
		  			'<div class="mui-btn mui-btn--raised mui-btn--primary" id="dialog-goGameOrFunGame-go">进入游戏</div>',
    		  		'<div class="mui-btn mui-btn--raised mui-btn--dark" id="dialog-goGameOrFunGame-fun">试玩游戏</div>',
    		  	'</div>',
    		'</div>',
    	'</div>'
    ].join('');
	
	_dialogConfigs['goGameOrFunGame'].initFn = function(dialog){
		
		dialog.$title = dialog.$el.find('#dialog-goGameOrFunGame-title');
		dialog.$message = dialog.$el.find('#dialog-goGameOrFunGame-message');
		
		dialog.$el.find('#dialog-goGameOrFunGame-go').click(function(){
			dialog.hide();
			if(typeof dialog.param.data.goGame === 'function'){
				dialog.param.data.goGame();
			}
		});
		
		dialog.$el.find('#dialog-goGameOrFunGame-fun').click(function(){
			dialog.hide();
			if(typeof dialog.param.data.goFun === 'function'){
				_dialog.param.data.goFun();
			}
		});
	}
	 ...
	 * @param {Object} param 参数
	 */
	_dialogConfigs['goGameOrFunGame'].defaultSetFn = function(dialog){
		var _config = {
			title:'输入标题',
			message:'内容',
			goGame:false,
		...
		$.extend(_config,dialog.param.data);
		dialog.param.data = _config;
		dialog.$title.html(_config.title);
		dialog.$message.html(_config.message);
	};
